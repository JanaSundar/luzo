...
interface EnvironmentState {
  ...
  getActiveEnvironmentDetailed: () => EnvironmentVariable[];
}
    ...
    immer<EnvironmentState>((set, get) => ({
      ...
      updateEnvironmentVariable: (envId, key, value, secret) =>
        set((state) => {
          ...
          if (v) {
            v.value = value;
            if (secret !== undefined) v.secret = secret;
          } else {
            env.variables.push({ key, value, enabled: true, secret: secret ?? false });
          }
        }),
