...
export function ensurePipelineFlowDocument(pipeline: Pipeline): FlowDocument {
  ...
  const existingEdges = (existing?.edges ?? []).filter(
  ...
  const edges =
    existingEdges.length > 0
      ...
      : buildDefaultFlowEdges(startNode.id, requestNodes, pipeline.steps);
...
function buildDefaultFlowEdges(
  startNodeId: string,
  requestNodes: FlowNodeRecord[],
  steps: PipelineStep[],
) {
  if (requestNodes.length === 0) return [];
  const requestEdges = buildRequestDependencyEdges(steps);
  const requestTargets = new Set(requestNodes.map((node) => node.id));
  const incomingCounts = new Map<string, number>();

  for (const edge of requestEdges) {
    if (!requestTargets.has(edge.target)) continue;
    incomingCounts.set(edge.target, (incomingCounts.get(edge.target) ?? 0) + 1);
  }

  const startEdges = requestNodes
    .filter((node) => (incomingCounts.get(node.id) ?? 0) === 0)
    .map((node) => createFlowEdge(startNodeId, node.id, "control"));

  return [...startEdges, ...requestEdges];
}
