  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Section title="Overview">{renderMarkdownToPdf(report.summary)}</Section>
          ...
          {report.requests.map((req, idx) => (
            <View key={idx} style={styles.requestCard} wrap={false}>
              ...
              <View style={{ borderLeftWidth: 1.5, borderLeftColor: "#e5e7eb", paddingLeft: 6 }}>
                {renderMarkdownToPdf(req.analysis)}
              </View>
            </View>
          ))}
        </Section>

        <Section title="Conclusion">{renderMarkdownToPdf(report.conclusion)}</Section>
    ...
    <View>
      {safeItems.map((item, index) => (
        <View key={`${item}-${index}`} style={styles.bulletRow} wrap={false}>
          ...
          <View style={{ flex: 1 }}>{renderMarkdownToPdf(item)}</View>
        </View>
      ))}
    </View>
  );
