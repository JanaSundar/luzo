...
type TimelineFilter = "all" | "failed" | "running" | "mock" | "condition";
...
function matchesFilter(event: TimelineEvent, filter: TimelineFilter): boolean {
  ...
  if (filter === "mock") return event.eventKind === "request" && event.isMock;
  if (filter === "condition") return event.eventKind === "condition";
  return true;
}
    ...
    <div className="lg:col-span-3 border-r flex flex-col min-h-0">
      ...
      <div className="border-b bg-muted/10 p-3">
        ...
        <div className="mt-2 flex items-center gap-1 overflow-x-auto pb-1">
          {(["all", "failed", "running", "mock", "condition"] as const).map((value) => (
            <button
              key={value}
              type="button"
        ...
        {activeRouteFocus && (
          <div className="mt-2 rounded-md border border-primary/20 bg-primary/5 px-2 py-1 text-[10px] text-primary">
            Branch focus: {activeRouteFocus.mode === "chosen" ? "Selected path" : "Skipped path"}
          </div>
        )}
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
        {filteredEvents.map((event) => (
          <TimelineEventRow
            ...
            isDimmed={highlightedEventIds.size > 0 && !highlightedEventIds.has(event.eventId)}
