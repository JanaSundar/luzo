...
export function collectStepDependencies(
  ...
  const templateDependencies = candidates.flatMap(({ field, value }) =>
  ...
  const explicitDependencies = (step.upstreamStepIds ?? [])
    .map((stepId) => aliasByStepId.get(stepId))
  ...
  return [...templateDependencies, ...explicitDependencies];
}
