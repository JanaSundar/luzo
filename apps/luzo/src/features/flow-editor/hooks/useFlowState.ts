...
import { ensureFlowDocument } from "../domain/flow-document";
import type { FlowBlock, FlowConnection } from "../domain/types";
import { toFlowEdge, toFlowNode } from "../nodeAdapters";

const DEFAULT_REQUEST: Omit<PipelineStep, "id" | "upstreamStepIds"> = {
  auth: { type: "none" },
  body: null,
  bodyType: "none",
  headers: [],
...
export function useFlowState(pipelineId: string | null) {
  ...
  const setFlowDocument = usePipelineStore((state) => state.setFlowDocument);
  const [selectedNodeIds, setSelectedNodeIds] = useState<string[]>([]);
  const [selectedEdgeIds, setSelectedEdgeIds] = useState<string[]>([]);
  const selectedNodeIdSet = useMemo(() => new Set(selectedNodeIds), [selectedNodeIds]);
  const selectedEdgeIdSet = useMemo(() => new Set(selectedEdgeIds), [selectedEdgeIds]);
  const flow = useMemo(
    () => (pipeline ? ensureFlowDocument(pipeline.flow, pipeline.steps) : null),
    [pipeline],
  );
  ...
  const updateFlow = useCallback(
    (updater: (current: NonNullable<typeof flow>) => NonNullable<typeof flow>) => {
      if (!pipelineId || !flow) return;
      setFlowDocument(pipelineId, updater(flow));
    },
    [flow, pipelineId, setFlowDocument],
  );
...
function createBlock(
  ...
  options: { aiModel: string; aiProvider: "openai" | "groq" | "openrouter" },
): FlowBlock {
  const id = crypto.randomUUID();

  switch (type) {
    case "request":
      return { id, type, position, data: { ...DEFAULT_REQUEST } };
    case "evaluate":
      return {
        id,
        type,
